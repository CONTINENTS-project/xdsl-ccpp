// Test the Python frontend → optimizer → Fortran pipeline for the helloworld
// example.  helloworld_py.py is the Python equivalent of hello_world_suite.xml
...
//
// RUN: python3 examples/helloworld/helloworld_py.py | python3 -m xdsl_ccpp.tools.ccpp_opt -p generate-meta-cap,generate-meta-kinds,generate-suite-cap,generate-ccpp-cap,generate-kinds,strip-ccpp -t ftn | python3 -m filecheck %s
